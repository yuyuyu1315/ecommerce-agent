...
| 知识问答（RAG） | 基于运营知识库与商品数据检索增强回答，带来源标注，附 36 条评测集与迭代指标 |
...
| 维度 | 内容 |
|---|---|
| 数据规模 | 54 万条真实交易 → 清洗聚合 → **300 商品 / 9 分类 / 900 竞品价** |
| 数据来源 | UCI Online Retail（电商公开基准数据集，官网可下载验证） |
...
### 选品回测（真实数据驱动，报告见 [`docs/selection_evaluation.md`](docs/selection_evaluation.md)）

用 UCI 真实销量做 ground truth，对比"规则基线 vs LLM 推荐"：
...
结论：数据驱动的规则评分显著优于随机/经验选择；LLM 基于数据上下文判断与真实热销高度一致，二者互补（规则负责"筛"，LLM 负责"评"）。

### RAG 评测（36 条真实问答，报告见 [`docs/rag_evaluation.md`](docs/rag_evaluation.md)）

30 条知识库内问答 + 6 条知识库外拒答测试，驱动检索策略迭代：
...
| 拒答正确率（知识库外） | 未测 | **100%** |
| 回答成功率 | 100% | 100% |
| 平均延迟 | 5675 ms | 5465 ms |

- 迭代过程：V1 商品文档挤占检索名额 → V2 分类型混合检索（未提升）→ V3 来源加权（知识库 ×1.25）显著提升。
- 拒答测试：知识库外问题（如"2026年世界杯冠军"）100% 诚实拒答不编造——AI 助手的安全底线。
